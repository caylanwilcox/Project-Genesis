...
async function test1MData() {
  ...
  console.log('-'.repeat(80));
  try {
    const response1h = await client.getStocksAggregates(
      ticker,
      1,
      'hour',
      fromDateStr,
    ...
    if (response1h.results && response1h.results.length > 0) {
      ...
      console.log(`  OHLC: O=$${first.o.toFixed(2)} H=$${first.h.toFixed(2)} L=$${first.l.toFixed(2)} C=$${first.c.toFixed(2)}`);
      ...
      console.log(`  OHLC: O=$${last.o.toFixed(2)} H=$${last.h.toFixed(2)} L=$${last.l.toFixed(2)} C=$${last.c.toFixed(2)}`);
      ...
      const lastBarDate = new Date(lastBarMs);
      const lastBarHour = lastBarDate.getUTCHours();
      const lastBarMinute = lastBarDate.getUTCMinutes();
      console.log(`Last bar hour (UTC): ${lastBarHour}:${String(lastBarMinute).padStart(2, '0')}`);
      console.log(`Expected market close (UTC): ~21:00 (4PM ET)`);
    } else {
  ...
  console.log('-'.repeat(80));
  try {
    const response1d = await client.getStocksAggregates(
      ticker,
      1,
      'day',
      fromDateStr,
