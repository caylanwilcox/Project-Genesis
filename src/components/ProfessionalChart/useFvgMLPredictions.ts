    ...
    const fetchPredictions = async () => {
      try {
        ...
        setPredictedPatterns(patterns.map(pattern => {
          ...
          if (pred) {
            // Ensure win_probability is a valid number, fallback to 0.5 if NaN/undefined
            const winProb = typeof pred.win_probability === 'number' && !isNaN(pred.win_probability)
              ? pred.win_probability
              : 0.5

            // Derive confidence tier if not provided
            const confidence = typeof pred.confidence === 'number' && !isNaN(pred.confidence) ? pred.confidence : winProb
            const confidenceTier = pred.confidence_tier ||
              (confidence >= 0.85 ? 'very_high' : confidence >= 0.7 ? 'high' : confidence >= 0.55 ? 'medium' : 'low')

            return {
              ...pattern,
              mlPrediction: {
                winProbability: winProb,
                recommendation: pred.recommendation || (winProb >= 0.7 ? 'TRADE' : winProb >= 0.5 ? 'CAUTIOUS' : 'SKIP'),
                confidenceTier: confidenceTier as 'very_high' | 'high' | 'medium' | 'low',
                modelAccuracy: typeof pred.model_accuracy === 'number' && !isNaN(pred.model_accuracy) ? pred.model_accuracy : 0.72,
              }
            }
          }
