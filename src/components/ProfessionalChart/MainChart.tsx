...
export const MainChart: React.FC<MainChartProps> = ({
  ...
  // Use requestAnimationFrame for smooth rendering synchronized with browser paint cycle
  ...
  useEffect(() => {
    ...
    // Cancel any pending frame to prevent stale renders
    if (rafIdRef.current) {
    ...
    const tags = drawPriceLines(ctx, rect, padding, chartHeight, minPrice, maxPrice, priceRange, stopLoss, entryPoint, targets, rangeHigh, rangeLow, rangeMid, retestHigh, retestLow)
    ...
    // Cleanup: cancel pending animation frame on unmount or dependency change
    return () => {
      if (rafIdRef.current) {
        cancelAnimationFrame(rafIdRef.current)
      }
