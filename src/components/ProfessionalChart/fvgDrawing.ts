...
export function drawFvgPatterns(
  ...
  patterns.forEach(pattern => {
    ...
    if (!pattern.expanded) {
      ...
      if (pattern.mlPrediction && typeof pattern.mlPrediction.winProbability === 'number' && !isNaN(pattern.mlPrediction.winProbability)) {
    ...
    const hasValidMLPrediction = pattern.mlPrediction &&
      typeof pattern.mlPrediction.winProbability === 'number' &&
      !isNaN(pattern.mlPrediction.winProbability)

    if (hasValidMLPrediction) {
      ...
      ctx.fillStyle = mlColor
      ctx.fillText(pattern.mlPrediction!.recommendation || 'N/A', labelX, labelY + 36)
    } else {
