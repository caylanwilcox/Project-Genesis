...
export function useChartInteraction(
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>, rect: DOMRect) => {
    if (isPanning && panStart) {
      ...
      // Standard: drag right = scroll right (see older data), drag left = scroll left (see newer data)
  ...
  const handleWheel = useCallback((e: React.WheelEvent<HTMLDivElement> | WheelEvent, rect: DOMRect) => {
    // Note: preventDefault is handled by native event listener in ChartInteractionHandlers
    if (!rect) return
    // TradingView-like multiplicative zoom: scroll up = zoom in (more detail), scroll down = zoom out (see more)
    ...
    console.log(`[🔍 ZOOM] ${currentScale.toFixed(2)} → ${newScale.toFixed(2)} (deltaY=${e.deltaY})`)
    applyZoomToScale(newScale, ratio)
